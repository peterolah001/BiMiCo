...
```{r a4, eval=F}
rawfqsF <- "../Demo_data_bimico/FWD_Illumina/"
rawfqsR <- "../Demo_data_bimico/REV_Illumina/"
```

#### File format: please specify *pattern=* if other than "fastq.gz"
```{r a4b, eval=F}
fastqFs <- sort(list.files(rawfqsF), pattern="fastq.gz")
fastqRs <- sort(list.files(rawfqsR), pattern="fastq.gz"))
```
...
```{r a8, eval=F}
filtered_fqs_FWD <- "../Demo_data_bimico/filt_FWD_Illumina"
filtered_fqs_REV <- "../Demo_data_bimico/filt_REV_Illumina"
```
